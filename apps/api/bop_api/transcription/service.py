...
* ``process_pending`` (called by a background thread, or directly by tests)
  claims one ``queued`` row at a time, extracts audio, runs the transcriber,
...
class TranscriptionService:
    ...
    def process_pending(self, *, max_segments: int | None = None) -> int:
        ...
        with self._processing_lock:
            processed = 0
            while max_segments is None or processed < max_segments:
    ...
    def latest_analyzed_by_recording(
        ...
        *,
        ...
        """Return the max ``incident_end_seconds`` of terminal rows per recording.

        Only ``completed`` and ``empty`` rows count toward the latest analyzed
        time so failed windows do not falsely advance the lag indicator.
        """
        if not recording_ids:
            return {}
        query = (
            ...
            .where(
                and_(
                    TranscriptSegment.run_id == run_id,
                    TranscriptSegment.recording_id.in_(recording_ids),
                    TranscriptSegment.status.in_(("completed", "empty")),
                )
            )
        )
        if session is not None:
        ...
        for recording_id, incident_end in rows:
            current = latest.get(recording_id)
            if current is None or incident_end > current:
